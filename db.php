 ...
 *
 * @param	bool	$btcnew_ignore_own_blog	The value we are toggling to
 * @param	array	$comment_IDs		Collection of comment IDs
 * @return	bool				True when successful; false otherwise
 */
function btcnew_db_update_ignored($btcnwe_ignore, $comment_IDs) {
	global $wpdb;
	foreach ($comment_IDs as $comment_ID) {
		if ($btcnew_ignore) {
			...
			$comment_approved = (get_option(BTCNEW_MODERATION_OPTION)) ? '0' : '1';
			$query = 'UPDATE ' . $wpdb->prefix . 'comments ' .
 ...
 */
function btcnew_db_update_filtered($comment_type, $toggle) {
	global $wpdb;
...
function btcnew_db_comment_counts($ID, $filters=null) {
	...
	if (isset($filters['srcs']) && !empty($filters['srcs'])) {
		...
		foreach ($filters['srcs'] as $src) {
			$filter_query .= '\'btcnew_' . $wpdb->escape($src) . '\',';
		}
 ...
 * @return	bool		True when successful; false otherwise
 */
function btcnew_db_get_latest_post_IDs() {
	global $wpdb;
	$query = 'SELECT ID ' .
 ...
 */
function btcnew_db_dupe_comment($commentdata) {
	global $wpdb;
 ...
 */
function btcnew_db_dupe_clean($comment_ID) {
    global $wpdb;
 ...
 * @return	bool			True when successful; false otherwise
 */
function btcnew_db_delete_dupe_comment($comment_ID) {
	global $wpdb;
 ...
 */
function btcnew_db_set_comment_counts($ID, $counts) {
	$btcnew_comment_counts = array();
	foreach ($counts as $count) {
		$btcnew_comment_counts[] = array('comment_src' => $count->comment_src, 'cnt' => $count->cnt, 'enabled' => $count->enabled);
	}
	if (!update_post_meta($ID, 'btcnew_comment_counts', $btcnew_comment_counts)) {
		add_post_meta($ID, 'btcnew_comment_counts', $btcnew_comment_counts, true);
	}
}
 ...
 */
function btcnew_db_get_comment_counts($ID) {
	$btcnew_comment_counts = get_post_meta($ID, 'btcnew_comment_counts', true);
	if (!is_array($btcnew_comment_counts)) {
		return false;
	}
	$counts = array();
	foreach ($btcnew_comment_counts as $count) {
 ...
 *
 * @uses	btcnew_db_get_post_IDs()
 *
 * @param	int		$ID		Post ID
 */
function btcnew_db_clear_comment_counts($ID=null, $wp_update=true) {
	if ($ID == null) {
		$posts = btcnew_db_get_post_IDs();
		foreach ($posts as $post_ID) {
			delete_post_meta($post_ID, 'btcnew_comment_counts');
			if (function_exists('wp_update_comment_count') && $wp_update) {
				wp_update_comment_count($post_ID);
			}
		}
	} else {
		delete_post_meta($ID, 'btcnew_comment_counts');
		if (function_exists('wp_update_comment_count') && $wp_update) {
 ...
 */
function btcnew_db_set_comment_summary($ID, $summary) {
	$btcnew_comment_summary = array();
	foreach ($summary as $s) {
		$btcnew_comment_summary[] = array('comment_src' => $s->comment_src, 'cnt' => $s->cnt, 'enabled' => $s->enabled);
	}
	if (!update_post_meta($ID, 'btcnew_comment_summary', $btcnew_comment_summary)) {
		add_post_meta($ID, 'btcnew_comment_summary', $btcnew_comment_summary, true);
	}
}
 ...
 */
function btcnew_db_get_comment_summary($ID) {
	$btcnew_comment_summary = get_post_meta($ID, 'btcnew_comment_summary', true);
	if (!is_array($btcnew_comment_summary)) {
		return false;
	}
	$summary = array();
	foreach ($btcnew_comment_summary as $s) {
 ...
 * @param	int		$ID		Post ID
 */
function btcnew_db_clear_comment_summary($ID=null) {
	if ($ID == null) {
		$posts = btcnew_db_get_post_IDs();
		foreach ($posts as $post_ID) {
			delete_post_meta($post_ID, 'btcnew_comment_summary');
		}
	} else {
		delete_post_meta($ID, 'btcnew_comment_summary');
	}
}
 ...
 * @return	bool			True on success; false otherwise
 */
function btcnew_db_enable_post($post)
{
	$btcnew_post = array('ID' => $post->ID,
						'post_date_gmt' => $post->post_date_gmt,
						'initial_import_date_gmt' => '0000-00-00 00:00:00',
						'last_import_date_gmt' => '0000-00-00 00:00:00',
						'hits' => 0, 'misses' => 0);
	return add_post_meta($post->ID, 'btcnew_post', $btcnew_post, true);
}
 ...
 * @return	bool			True on success; false otherwise
 */
function btcnew_db_disable_post($ID)
{
	return delete_post_meta($ID, 'btcnew_post');
}
 ...
 */
function btcnew_db_get_post_IDs() {
	global $wpdb;
	$query = 'SELECT post_id ' .
			'FROM ' . $wpdb->prefix . 'postmeta ' .
			'WHERE meta_key=\'btcnew_post\'';
	return $wpdb->get_col($query);
}
 ...
 * @return	bool			True when successful; false otherwise
 */
function btcnew_db_update_post($post) {
	$btcnew_post = array('ID' => $post->ID,
						'post_date_gmt' => $post->post_date_gmt,
	...
	return update_post_meta($post->ID, 'btcnew_post', $btcnew_post);
}
 ...
 *
 * @global	object	$wpdb	DB object
 * @uses	btcnew_db_enable_post()
 * @uses	btcnew_db_get_latest_post_IDs()
 *
 * @return	array	Collection of post objects
 */
function btcnew_db_get_posts() {
	global $wpdb;
	$IDs = btcnew_db_get_latest_post_IDs();
	
	$btcnew_posts = array();
	foreach ($IDs as $ID) {
		if (!$btcnew_post = get_post_meta($ID, 'btcnew_post', true)) {
			$post = $wpdb->get_row($wpdb->prepare('SELECT ID,post_date_gmt FROM ' . $wpdb->prefix . 'posts WHERE ID=%s', $ID));
			btcnew_db_enable_post($post);
			$btcnew_post = array('ID' => $ID,
								'post_date_gmt' => $post->post_date_gmt,
								'initial_import_date_gmt' => '0000-00-00 00:00:00',
								'last_import_date_gmt' => '0000-00-00 00:00:00',
								'hits' => 0, 'misses' => 0);
		}
		$btcnew_posts[] = (object) $btcnew_post;
	}
	return $btcnew_posts;
}
 ...
 * @return	bool					True when successful; false otherwise
 */
function btcnew_db_add_own_blog_comment($ID, $comment_ID) {
	return add_post_meta($ID, 'btcnew_own_blog_comment', $comment_ID);
}
 ...
 * @return	bool					True when successful; false otherwise
 */
function btcnew_db_add_retweet($ID, $comment_ID) {
	return add_post_meta($ID, 'btcnew_retweet', $comment_ID);
}
 ...
 */
function btcnew_db_get_own_blog_comments() {
	$IDs = btcnew_db_get_post_IDs();
	$comment_IDs = array();
	foreach ($IDs as $ID) {
		$comment_IDs = array_merge($comment_IDs, get_post_meta($ID, 'btcnew_own_blog_comment'));
	}
	return $comment_IDs;
}
 ...
 * @return	bool	Collection of comment IDs
 */
function btcnew_db_get_retweets() {
	$IDs = btcnew_db_get_post_IDs();
	$comment_IDs = array();
	foreach ($IDs as $ID) {
		$comment_IDs = array_merge($comment_IDs, get_post_meta($ID, 'btcnew_retweet'));
	}
	return $comment_IDs;
}
 ...
 */
function btcnew_db_clear_own_blog_comments() {
	global $wpdb;
	$query = 'DELETE FROM ' . $wpdb->prefix . 'postmeta ' .
			'WHERE meta_key=\'btcnew_own_blog_comment\'';
	return $wpdb->query($query);
}
 ...
 * @return	bool	DESCRIPTION
 */
function btcnew_db_clear_retweets() {
	global $wpdb;
	$query = 'DELETE FROM ' . $wpdb->prefix . 'postmeta ' .
			'WHERE meta_key=\'btcnew_retweet\'';
	return $wpdb->query($query);
}
 ...
 * @param	int		$comment_ID		Comment ID
 */
function btcnew_db_add_comment_type($ID, $comment_type, $comment_ID) {
 ...
 * @param	int		$comment_ID		Comment ID
 */
function btcnew_db_clear_comment_type($ID, $comment_type, $comment_ID) {
	global $wpdb;
	$query = 'DELETE FROM ' . $wpdb->prefix . 'postmeta ' .
 ...
 * @return	bool					True when successful; false otherwise
 */
function btcnew_db_clear_all_comment_type($comment_type) {
	global $wpdb;
	$query = 'DELETE FROM ' . $wpdb->prefix . 'postmeta ' .
			'WHERE meta_key=%s';
 ...
 * @return	bool			True when successful; false otherwise
 */
function btcnew_db_clear_posts() {
	global $wpdb;
	$query = 'DELETE FROM ' . $wpdb->prefix . 'postmeta ' .
			'WHERE meta_key=\'btcnew_post\'';
	return $wpdb->query($query);
